...
1. Choose a stored record in the **Mission explorer** sidebar. **New folder** creates a folder in the current location; open it to create subfolders. Use breadcrumbs or the up arrow to navigate. The **⋯** button renames/moves a folder or moves a log; logs can also be dragged onto a folder or breadcrumb. Search spans all folders and missions. Folder organisation is shared by authenticated sessions on this WISL server and saved in SQLite, without moving original log files or changing evidence. On small screens, use **Flight library** to open the drawer. The last selected flight and browsed folder return when you reload the browser session.
