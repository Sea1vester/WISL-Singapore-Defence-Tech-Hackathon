...
**Date:** 13 September 2026 (updated)
...
The latest 90-file simulator audit projected 90/90 files to non-null, finite, schema-valid L2 and completed every detector path within the chosen 30-second bound. Expectations were met for 40/90 files. The API suite passed 161 tests in 5.92 seconds; replay tests passed 15, UI tests passed 5, and parser tests passed 28 in 0.73 seconds. Separately, a physically gated synthetic set of 36 exports, representing eight seeded failure missions and one normal control across four format exports each, achieved 36/36 structural validity and declared expectation hits; normal-control exports produced no incidents.
...
The API suite result is 161 passed with four deprecation warnings in 5.92 s. Replay Node tests are 15 passed, UI Node tests are five passed, and parser tests are 28 passed in 0.73 s. The latest audit of the original hazards-only 90-file inventory projected 90/90 files to non-null, finite, schema-valid L2 and completed all detector paths within the chosen 30-second bound, with no whole-path timeouts. Deterministic scenario expectations were met for 40/90 files. This remains simulator and software evidence only. The original corpus has no normal control and only one exact GPS-weak mission. A separate physically gated synthetic audit contains 36 exports: eight seeded failure missions and one normal control, each represented by four format exports. It projected 36/36 files to valid L2, met 36/36 declared scenario expectations and had no whole-path timeouts at the chosen 30-second bound; the normal-control exports produced no incidents. Its fixture-soundness gate checks battery-voltage and ground-altitude coupling, reference-profile novelty, parser round-trip and normal-cadence velocity consistency. The canonical audit separately checks finite values and non-decreasing timestamps. These format exports are non-independent representations of synthetic missions and do not establish detector accuracy, operational physical validity, recurrence, physical cause or field performance. Its isolated two-representative upload-to-ready/replay/incident-query path completed in 1.223 s; this is a single local software-path observation, not throughput.
...
The frozen validation artefacts are `output/evidence/corpus-validation-v2/original90/corpus-validation.json` and `output/evidence/corpus-validation-v2/failure-fixtures-audit/corpus-validation.json`. They separately record the original hazards-only corpus and the gated synthetic failure-plus-control set. The report builder is `scripts/build_wisl_final_report.py`; it creates the companion PDF from the text and report layout. This appendix is excluded from the ten-page body budget.
