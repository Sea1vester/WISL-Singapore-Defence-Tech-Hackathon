...
def build():
    ...
    meta = "<b>Team</b><br/>Inessa Wong - NUS Mathematics and Computer Science<br/>Sylvester Lim - NUS Computer Science<br/>Isaac Sua - NUS Computer Engineering; DSO research intern<br/><br/><b>Mentor/advisor</b><br/>Unknown at time of submission draft<br/><br/><b>Date</b><br/>13 September 2026 (updated)"
    ...
    body_page(s, "5. Test methodology", "PRE-SPECIFIED SIMULATOR-CORPUS AUDIT", [
        ...
        ("p", "The original directory contains no normal/control mission. The separate fixture-soundness gate checks battery-voltage/ground-altitude coupling, profile novelty, parser round-trip and normal-cadence velocity consistency; the canonical audit separately checks finite values and monotonic timestamps. Neither establishes operational physical validity. The material does not cover RF contest, actual comms loss, weather, low light, clutter, vibration, thermal stress, adversarial inputs or field workflow."),
        ("band", "API suite: 161 passed, four deprecation warnings, 5.92 s. Replay Node: 15 passed; UI Node: five passed; parser: 28 passed in 0.73 s. Latest original audit: 90/90 valid L2/detector-complete, zero 30-s whole-path timeouts, 40/90 expected hits. Separate gated synthetic set: 36/36 valid/expected, zero 30-s timeouts (eight seeded failures + one normal control, four format exports each); normal-control exports had zero incidents. Two isolated E2E representatives: 1.223 s. Simulator/software evidence only."),
    ], styles)
    ...
    s += [P("Frozen validation artefacts: output/evidence/corpus-validation-v2/original90/corpus-validation.json and output/evidence/corpus-validation-v2/failure-fixtures-audit/corpus-validation.json. They separately record the original hazards-only corpus and gated synthetic failure-plus-control set. This report's builder is scripts/build_wisl_final_report.py. It uses reportlab and fixed page boundaries: cover, ten body pages, references and appendix.", styles["body"]), band("Interpret all generated corpus scenarios as synthetic/SITL fixtures. No operational field-flight conclusion may be inferred from this appendix. Supplemental synthetic controls remain outside the original corpus denominator.", styles, RED)]
    doc.build(s)
